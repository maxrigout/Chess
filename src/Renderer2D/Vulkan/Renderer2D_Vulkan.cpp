...
void Renderer2D_Vulkan::End()
{
	...
	uint32_t imageIndex;
	int result = vkAcquireNextImageKHR(m_device, m_swapChain, UINT64_MAX, m_imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);
...
{
	auto vertShaderCode = readFile("shaders/glsl/byt/vert.spv");
	auto fragShaderCode = readFile("shaders/glsl/byt/frag.spv");
...
{
	VkDeviceSize bufferSize = ARRAY_LEN(vertices) * sizeof(vertices[0]);
